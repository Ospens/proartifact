...
class News < ApplicationRecord
  ...
  has_many :votes_down, -> { where(vote: -1, likeable_type: 'News') }, class_name: 'Like', foreign_key: :likeable_id
  has_many :votes_up, -> { where(vote: 1, likeable_type: 'News') }, class_name: 'Like', foreign_key: :likeable_id

  has_attached_file :title_image,
                    styles: { medium: '400x400#', large: '1000x500#', thumb: '750x439#',
                              thumb_small: '380x220#', small: '260x180#' },
                    default_url: '/images/:style/missing.png',
                    path: ':rails_root/public/images/news/:id/:attachment/:style/:filename',
                    url: '/images/news/:id/:attachment/:style/:filename',
                    use_timestamp: false

  validates_attachment_content_type :title_image, content_type: %r{\Aimage/.*\z}

  validates :title, :news_text, :publish_date, :short_description, :title_image, presence: true
